document.addEventListener('DOMContentLoaded', () => {
    ...
    (function(index) {
        ...
        SQUARES[i].onclick = function(){
            //if the square below the current square is taken, go on top of it
            if (SQUARES[index + 7].classList.contains('taken')) {
                if (currentPlayer === 1) {
                    SQUARES[index].classList.add('taken')
                    SQUARES[index].classList.add('player-one')
                    currentPlayer = 2
                    DISPLAY_CURRENT_PLAYER.innerHTML = currentPlayer
            ...
            //if the square below the current square is not taken, can't go there
            } else alert('no no no no no')
        }
    })(i)
    ...
    function checkBoard() {
        ...
        //take the 4 values in each winning array & plug them into the squares values
        for (let y = 0; y < WINNING_ARRAYS.length; y++) {
            const SQUARE1 = SQUARES[WINNING_ARRAYS[y][0]]
            const SQUARE2 = SQUARES[WINNING_ARRAYS[y][1]]
            const SQUARE3 = SQUARES[WINNING_ARRAYS[y][2]]
            const SQUARE4 = SQUARES[WINNING_ARRAYS[y][3]]
            ...
            //check those arrays to see if they all have the class of player-two
